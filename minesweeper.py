    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                if 0 <= i < self.height and 0 <= j < self.width:
                    n_cell = (i, j)
                    if (n_cell in self.mines) or (n_cell in self.safes):
                        continue
                    n_sentence.add(n_cell)

        if n_sentence:
            self.knowledge.append(Sentence(n_sentence, count))
        # 4) mark any additional cells as safe or as mines
        #    if it can be concluded based on the AI's knowledge base
        for sentence in self.knowledge:
        ...
        for sentence1 in knowledge_copy:
            ...
            for sentence2 in knowledge_copy:
                ...
                if sentence1.cells.issubset(sentence2.cells):
                    ...
                    print(sentence2.cells,sentence2.count,"-",sentence1.cells,sentence1.count)
                    if new_sentence in self.knowledge or len(new_sentence_cells) == 0:
                        continue
                    self.knowledge.append(new_sentence)
                    
        self.knowledge = [x for x in self.knowledge if x not in remove]
